...
document.addEventListener('DOMContentLoaded', function() {
    // Navigation des sections (sticky + offcanvas) — sans ancien modal
    ...
    document.querySelectorAll('#sectionsNav .nav-link, #sectionsOffcanvas .nav-link').forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const scrollToTarget = () => {
                ...
                if (scrollSpyInstance) scrollSpyInstance.refresh();
            };
            const offcanvasEl = document.getElementById('sectionsOffcanvas');
            const offcanvas = offcanvasEl ? bootstrap.Offcanvas.getInstance(offcanvasEl) : null;
            if (offcanvas) {
                const handler = () => { offcanvasEl.removeEventListener('hidden.bs.offcanvas', handler); scrollToTarget(); };
...
// Aperçu structuré et helpers retirés (orchestrateur EDxoTasks gère l'affichage)
