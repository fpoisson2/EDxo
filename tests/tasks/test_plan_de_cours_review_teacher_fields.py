    ...
    class _Stream:
        ...
        def get_final_response(self):
            ...
            output_parsed = {
                ...
                'materiel': '',
                'nom_enseignant': 'Mme Nouvelle',
                'telephone_enseignant': '555-0000',
                'courriel_enseignant': 'new@example.com',
                'bureau_enseignant': 'C-200',
                'calendriers': [],
                'disponibilites': [],
                'mediagraphies': [],
                'evaluations': [],
...
def test_review_includes_teacher_contact_fields(app, client):
    with app.app_context():
        ...
        plan = PlanDeCours(cours_id=cours.id, session='A25', nom_enseignant='M. Ancien', telephone_enseignant='555-9999', courriel_enseignant='old@example.com', bureau_enseignant='B-100'); db.session.add(plan); db.session.flush()
        ...
        with patch('src.app.routes.plan_de_cours.AsyncResult', return_value=DummyAsync(result)):
            ...
            resp = client.get(f'/plan_de_cours/review/{plan.id}?task_id=tid-tch')
            assert resp.status_code == 200
            html = resp.get_data(as_text=True)
            for label in ['Nom enseignant', 'Téléphone enseignant', 'Courriel enseignant', 'Bureau enseignant']:
